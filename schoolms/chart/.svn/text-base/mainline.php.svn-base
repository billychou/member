<?php   
 ...
 foreach($output as $thismonth){
         ...
         $sale[$month]=number_format((float)$thismonth['salesvalue']/10000, 2, '.', '');
         $profit[$month]=number_format((float)$thismonth['salerevenue']/10000, 2, '.', '');
         $debt[$month]=number_format((float)$thismonth['ownmoney']/10000, 2, '.', '');
         $driver[$month]=number_format((float)$thismonth['driverprice']/10000, 2, '.', '');
 }
 //var_dump($output);
 //exit;
 ...
 $myPicture->setFontProperties(array("FontName"=>"fonts/SIMHEI.TTF","FontSize"=>7,"R"=>4,"G"=>98,"B"=>172));
 $myPicture->drawText(10,24,"营收状况(销售部+物流部)逐月走势总览",array("FontSize"=>12,"Align"=>TEXT_ALIGN_BOTTOMLEFT));

   /* Write the chart title */ 
 $myPicture->setFontProperties(array("FontName"=>"fonts/SIMHEI.TTF","FontSize"=>10,"R"=>0,"G"=>0,"B"=>0));
 $myPicture->drawText(280,65,"营收走势图("."20".$year."年度)",array("FontSize"=>13,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE));

 /* Set the default font */
 $myPicture->setFontProperties(array("FontName"=>"fonts/SIMHEI.TTF","FontSize"=>9,"R"=>0,"G"=>0,"B"=>0));


 /* Define the chart area */
 $myPicture->setGraphArea(42,65,500,260);
 ...
 $myPicture->drawLegend(180,285,array("Style"=>LEGEND_NOBORDER,"Mode"=>LEGEND_HORIZONTAL,"FontR"=>0,"FontG"=>0,"FontB"=>0));
