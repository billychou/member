...
function get_sales_stat_month_info_total() {
        $year = date("Y");
        $conn = db_connect();
        //$query = "select * from mergelist where listindex ='".$listindex."'";
                /* 
		$query = "select" 
				...
				." from billoflading"
				." left join importlist on billoflading.importlistid = importlist.id"
                                ." group by substring(billoflading.updatetime,3,5)";
                 */
                 $query = "select"
				  ." substring(updatetime,3,5) as month,"
				  ." sum(takeamount) as takeamount,"
				  ." sum(realamount) as realamount,"
				  ." sum(receiveamount) as receiveamount,"
				  ." sum(applypay) as applypay,"
				  ." sum(ownpay) as ownpay,"
				  ." sum(driverpay) as driverpay,"
				  ." sum(salesrevenue) as salesrevenue"
				." from billofladingv  "
                                ." where 1=1 "
                                ." and substring(updatetime,1,4) = $year"
				  ." group by substring(updatetime,3,5)";
	        //echo $query;	
		$result = @$conn->query($query);
        if (!$result) {
                return false;
...
function get_sales_stat_period_info($startDate, $endDate) {
		...
		$query = "SELECT billofladingv.department,"
					  ." sum(takeamount) as takeamount,"
				  ." sum(realamount) as realamount,"
				  ." sum(receiveamount) as receiveamount,"
				  ." sum(applypay) as applypay,"
				...
				." from billofladingv  "
					...
					." 1=1";
		if($startDate != null) {
			$query = $query." and billofladingv.updatetime >= '".$startDate." 00:00:00'";
		}		
		
		if($startDate != null) {
			$query = $query." and billofladingv.updatetime <= '".$endDate." 23:59:59'";
		}	
					
					$query = $query." group by billofladingv.department;";
...
function get_purchase_stat_month_info_total() {
        $year = date("Y");
        $conn = db_connect();
        //$query = "select * from mergelist where listindex ='".$listindex."'";
...
function get_purchase_stat_period_info($startDate, $endDate) {
        $conn = db_connect();
        //$query = "select * from mergelist where listindex ='".$listindex."'";
        
		$query = "SELECT importlistv.department,"
					." sum(initvalue) as initvalue,"
				  ." sum(purchasevalue) as purchasevalue"
				." from importlistv  "
					." where "
					." 1=1";
		if($startDate != null) {
			$query = $query." and importlistv.updatetime >= '".$startDate." 00:00:00'";
		}		
		
		if($startDate != null) {
			$query = $query." and importlistv.updatetime <= '".$endDate." 23:59:59'";
		}	
					
					$query = $query." group by importlistv.department;";
...
function get_log_list() {
   ...
   $query = "select 
			...
			admin.sys_role
			from log
			left join admin on log.userid = admin.id
			where 1=1
			";
	if(!canAdm()) {
			$currentUser = $_SESSION['thisuser'];
			$query = $query."and log.userid = ".$currentUser['id'];
	}
...
function get_bill_customer_period_list($intervalday,$customername) {
   // query database for a list of concrete
   $conn = db_connect();
   ...
   $query = "select a.*,
				(thisPerMount - lastPerMount) as delta1,
				(per3Mount - lastPerMount) as delta2,
				(per4Mount - per3Mount) as delta3,
				(per5Mount - per4Mount) as delta4
				from (
				select this.customerid,
				 this.customername,
				 case when this.thisPerMount is null then 0 else this.thisPerMount end as thisPerMount,
				 case when last.lastPerMount is null then 0 else last.lastPerMount end as lastPerMount,
				 case when per3.per3Mount is null then 0 else per3.per3Mount end as per3Mount,
				 case when per4.per4Mount is null then 0 else per4.per4Mount end as per4Mount,
				 case when per5.per5Mount is null then 0 else per5.per5Mount end as per5Mount
				 from
				(SELECT customerid, customername, sum(realamount) as thisPerMount FROM billcustomerstatv where" 
				." intervalday <= ".$intervalday
				." group by customerid,customername
				) this
				 left join 
				(SELECT customerid, customername, 
				sum(realamount) as lastPerMount FROM billcustomerstatv where 
				 1=1"
				." and intervalday  > ".$intervalday
				." and intervalday <= ".$intervalday * 2
				." group by customerid,customername) last "
				." on this.customerid = last.customerid"
				
				 ." left join 
				(SELECT customerid, customername, sum(realamount) as per3Mount FROM billcustomerstatv where 
				 1=1"
				." and intervalday  > ".$intervalday * 2
				." and intervalday <= ".$intervalday * 3
				." group by customerid,customername) per3 "
				." on this.customerid = per3.customerid"
				 ." left join 
				(SELECT customerid, customername, sum(realamount) as per4Mount FROM billcustomerstatv where 
				 1=1"
				." and intervalday  > ".$intervalday * 3
				." and intervalday <= ".$intervalday * 4
				." group by customerid,customername) per4 "
				." on this.customerid = per4.customerid"
				." left join 
				(SELECT customerid, customername, sum(realamount) as per5Mount FROM billcustomerstatv where 
				 1=1"
				." and intervalday  > ".$intervalday * 4
				." and intervalday <= ".$intervalday * 5
				." group by customerid,customername) per5 "
				." on this.customerid = per5.customerid) as a where 1=1";
				
		if($customername != "") {
			$query = $query." and a.customername like '%".$customername."%'";
		}		
