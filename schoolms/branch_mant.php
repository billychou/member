...
<?php
	...
	if($action == "save") {
		...
		$id = $_GET['id'];
		$thisbranch = get_branch_by_id($id);		
		$id = $thisbranch['id'];
		$name = $thisbranch['name'];
		$owner = $thisbranch['owner'];
		$note = $thisbranch['note'];
	}
...
<div class="zt">
 ...
 <fieldset> 
  <legend>维护分校信息</legend>
  <form action="php/activate_branch.php" method="post" class="table_wb" id="purunit">
  <input type="hidden" name="id" id="id" value="<?php echo $id ?>"/>
  <input type="hidden" name="action" id="action" value="<?php echo $action ?>"/>
<table  align="center"   cellpadding="0" cellspacing="0"   class="biaoge">
  <tr >
    <td width="10%"class="jy">分校名称</td>
  ...
  <tr >
    <td width="10%"class="jy">分校负责人</td>
    <td width="20%"><input name="owner" id="owner" type="text" value="<?php echo $owner ?>"  maxlength="50" /> </td>
    <td style="border-left:0px;"><div id='ownerTip'style="width:150px"></div></td>
  </tr>
  <tr >
