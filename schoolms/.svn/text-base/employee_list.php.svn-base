...
?>
	...
	<script type="text/javascript" charset="utf-8">
			...
			$(document).ready(function() {
				...
				var oTable = get_list_init(220,0,0);
				...
				$("#edit_btn").click(function(){				
					...
					if(anSelected.length == 0) {
						...
						location.href = "employee_mant.php?action=edit&id="+selectedId;
					}
				});
...
<div class="zt">
 <fieldset> 
  <legend>员工信息
	...
	?>
	&nbsp;&nbsp;
	<a href="employee_mant.php?action=save"><img src="images/add-.gif" title="添加信息" border="0"/></a>
	<?php
		}
		if(canAdm()) {
	?>
...
<table cellpadding="0" cellspacing="0" border="0"  id="example" width="100%">
	...
	<tfoot>
		<tr class="highlighted">
			...
			<th><input type="text" name="按证件号搜索" value="按证件号搜索" class="search_init" /></th>
			<th><input type="text" name="按联系方式搜索" value="按联系方式搜索" class="search_init" /></th>
	...
	<tbody>		
			...
			?>
				<tr id="<?php echo $employee['id']?>">
					<td>
						<?php echo $employee['updatetime'] ?>
					</td>
					
					<td>
						<?php echo $employee['branchname'] ?>
					</td>
					...
					<td><?php echo $employee['creditnum'] ?></td>
					<td><?php echo $employee['contact'] ?></td>
					<td><?php echo $employee['onbroadtime'] ?></td>
				</tr>
			<?php	
                                }
                        }
