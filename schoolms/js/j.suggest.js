	(function($) {
		...
		$.suggest = function(input, options) {
			...
			if($.trim($input.val())=='' || $.trim($input.val())=='中文/拼音/首字母') $input.val('中文/拼音/首字母').css('color','#aaa');
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					for(h in options.hot_list){
						html+='<li rel="'+options.hot_list[h][0]+'"><a href="#'+h+'"><span>'+'&nbsp;</span>'+options.hot_list[h][1]+'</a></li>';
					}
					html='<div class="gray ac_result_tip">请输入中文/拼音/首字母或者↑↓选择</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
					...
					for (var i = 0; i < options.source.length; i++) {//国内城市匹配
						...
						if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2]) || reg.test(options.source[i][3])) {
							html += '<li rel="' + options.source[i][0] + '"><a href="#' + i + '"><span>' + '&nbsp;</span>' + options.source[i][1] + '</a></li>';
						}
					}
