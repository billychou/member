<?php
  //header('Content-Type:text/html;charset=utf-8');
	...
	$onbroadtime = isset($_POST['onbroadtime']) ? $_POST['onbroadtime'] : "";
	...
	if($action == "save") {  //保存客户信息
		if(insert_employee($name,$gender,$title,$creditnum,$contact,$onbroadtime,$branchid,$updatetime)) { //保存成功
		        $result = array("code"=>"1","info"=>"注册成功, 马上转入列表界面");								
		} else { //保存失败
			$result = array("code"=>"2","info"=>"注册失败, 请仔细核对输入信息");
		}
	} else if($action == "edit"){ //更新客户信息
		if(update_employee_by_id($id,$name,$gender,$title,$creditnum,$contact,$onbroadtime,$branchid,$updatetime)) {  //更新成功
				$result = array("code"=>"1","info"=>"更新成功, 马上转入列表界面");
		} else {
			$result = array("code"=>"2","info"=>"更新失败, 请仔细核对输入信息"); //更新失败
        }
	} else if($action == "delete"){ //删除客户信息
		if(delete_employee_by_id($id,$updatetime)) {  //删除成功
