<?php
	...
	$aColumns = array(
                'username','account_id','sys_role_desc','branchname','updatetime','opearttype','ip');
    ...
    */
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			...
			if($i==4) {
				$sWhere .= "`".$aColumns[4]."` >= '".$gaSql['link']->real_escape_string($_GET['sSearch_'.$i])." 00:00:00' "; // 处理开始时间
			} else if($i==6) {
				$sWhere .= "`".$aColumns[4]."` <= '".$gaSql['link']->real_escape_string($_GET['sSearch_'.$i])." 23:59:59' "; // 处理截止时间
			} else {
